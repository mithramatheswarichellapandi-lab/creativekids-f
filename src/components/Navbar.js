...
function Navbar() {
  ...
  className={`relative group transition duration-300 ${colors[i % colors.length]}`}
>
  <span className="group-hover:scale-105 transition">{item}</span>
  <span className="absolute left-0 -bottom-1 h-[2px] w-0 
    ...
    transition-all duration-300 group-hover:w-full"></span>
          ...
          {mobileMenu && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 right-0 w-64 sm:w-72 h-full bg-white/95 backdrop-blur-lg shadow-2xl z-40 p-6 flex flex-col gap-6"
              >
                <div className="flex flex-col gap-4 mt-10">
                  {menuItems.map((item, i) => (
                    <a
                      key={item}
                      href={`#${item.toLowerCase()}`}
                      className={`text-lg font-semibold ${colors[i % colors.length]} hover:scale-105 transition-transform`}
                      onClick={() => setMobileMenu(false)}
                    >
                      {item}
                    </a>
      ...
      {/* Enquiry Modal */}
      <EnquiryModal show={showModal} onClose={() => setShowModal(false)} />
    </>
  );
}
